...
# Gunicorn post-fork hook (optional)
...
# ✅ Auto-start scanner even when running under Gunicorn
def start_gunicorn_background():
    try:
        start_background_scanner()
        logger.info("🟢 Gunicorn-compatible background scanner started.")
    except Exception as e:
        logger.error(f"❌ Failed to start background scanner: {e}")

# 🔁 Activate scanner in all cases (local + Render/Gunicorn)
start_gunicorn_background()
